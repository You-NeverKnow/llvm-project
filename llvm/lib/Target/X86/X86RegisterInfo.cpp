...
void
X86RegisterInfo::eliminateFrameIndexMEF(MachineBasicBlock::iterator II,
                                     int SPAdj, unsigned FIOperandNum,
                                     RegScavenger *RS) const {
  MachineInstr &MI = *II;
  MachineFunction &MF = *MI.getParent()->getParent();
  ...
  unsigned BasePtr;
  if (MI.isReturn()) {
    assert((!needsStackRealignmentMEF(MF) ||
           MF.getFrameInfo().isFixedObjectIndex(FrameIndex)) &&
           "Return instruction can only reference SP relative frame objects");
    FIOffset = TFI->getFrameIndexReferenceSP(MF, FrameIndex, BasePtr, 0);
  } else {
    FIOffset = TFI->getFrameIndexReferenceMEF(MF, FrameIndex, BasePtr);
  }
...
Register X86RegisterInfo::getFrameRegisterMEF(const MachineFunction &MF) const {
  const X86FrameLowering *TFI = getFrameLowering(MF);
  return TFI->hasFPMEF(MF) ? FramePtr : StackPtr;
}
